...
5. 长期记忆：memory 工具写 MEMORY.md（项目事实）与 USER.md（用户画像），
   跨会话生效（冻结快照注入）。
6. 沙箱工具：读写/编辑/搜索/执行命令，带路径越界防护、超时与截断；
   批内全只读才并发，含写严格串行保因果。
7. 上下文管理：token 预算 + 三级压缩 + 大结果落盘 + usage 动态校准。
8. 交互式多会话：REPL 内可 /new /switch /del /save /load。
9. 计划先行：先制定计划，人工确认（可多轮修订/取消）后才执行。
10. 声明式技能：.agents/skills/ 下放 SKILL.md 即可为 agent 增加能力，
    invoke_skill 按需加载，启动只注入轻量清单。
11. 错误自愈：失败/拒绝结果回喂模型修正。
12. 终止条件：无工具调用即完成、达最大步数收尾、异常退出。

四、架构
核心为「思考→行动→观察」循环：调模型→解析工具→本地执行→回填结果，
直至模型不再调用工具。
