...
5. 工具本地沙箱执行：读写文件、定点编辑、搜索、执行命令，带路径越界
   防护、命令超时与输出截断。
6. 上下文管理：token 预算 + 三级压缩（截断/摘要/丢弃）+ 过载保护。
7. 交互式多会话：REPL 内可 /new /switch /del 多个会话，/save /load 支持
   单会话或全量持久化。
8. 计划先行：先制定计划，人工确认（可多轮修订/取消）后才执行。
9. 错误自愈：失败/拒绝结果回喂模型，使其自主修正。
10. 终止条件：无工具调用即完成、达最大步数收尾、异常退出。
